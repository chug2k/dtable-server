...
class HttpService {
  ...
  init() {
    ...
    app.get('/dtables/:dtable_uuid', (req, res) => {
      ...
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to get data from the current table.'});
        return;
      }
    ...
    app.get('/api/v1/dtables/:dtable_uuid/rows/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let { table_name : tableName, view_name : viewName, table_id : tableId, view_id : viewId, grouping : grouping } = req.query;
        let table, view;
        ...
        if (grouping === 'true') {
          ...
          res.send({'groups': dtableManager.listTableViewGroupedRows(dtable, table, view)});
          return;
        } else {
          res.send({'rows': dtableManager.listTableViewRows(dtable, table, view)});
          return;
        }
      });
    });
    ...
    app.post('/api/v1/dtables/:dtable_uuid/rows/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        try {
          ...
          dtableManager.insertRowToTable(username, app_name, dtable_uuid, dtable, operations, (isValid, error_message) => {
            if (!isValid) {
    ...
    app.put('/api/v1/dtables/:dtable_uuid/rows/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        try {
          let operations = req.body;
          dtableManager.modifyTableRow(username, app_name, dtable_uuid, dtable, operations, (isValid, error_message) => {
            if (!isValid) {
              res.status(400).send(error_message);
              return;     
            }
    ...
    app.delete('/api/v1/dtables/:dtable_uuid/rows/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        try {
          let operations = req.body;
          dtableManager.deleteTableRow(username, app_name, dtable_uuid, dtable, operations, (isValid, error_message) => {
            if (!isValid) {
              res.status(400).send(error_message);
              return;     
            }
    ...
    app.post('/api/v1/dtables/:dtable_uuid/links/', (req, res) => {
      // update row's links
      let { dtable_uuid } = req.params;
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload || payload.permission !== 'rw') {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        try {
          let linkParams = req.body;
          dtableManager.addLinkToTableRow(username, app_name, dtable_uuid, dtable, linkParams, (isValid, error_message) => {
            if (!isValid) {
              res.status(400).send(error_message);
              return;     
            }
    ...
    app.delete('/api/v1/dtables/:dtable_uuid/links/', (req, res) => {
      // delete row's links
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        try {
          let linkParams = req.body;
          dtableManager.deleteLinkFromTableRow(username, app_name, dtable_uuid, dtable, linkParams, (isValid, error_message) => {
            if (!isValid) {
              res.status(400).send(error_message);
              return;     
            }
    ...
    app.get('/api/v1/dtables/:dtable_uuid/filtered-rows/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let { filters : filters, filter_conjunction : filterConjunction } = req.body;
        let table, view;
        ...
        filters.map(filter => {
          let column = TableUtils.getTableColumnByName(table, filter.column_name);
          if (!column) {
            res.status(400).send({"error_msg": `column ${filter.column_name} not found`});
            return;
          }
          filter.column_key = column.key;
        });

        res.send({'rows': dtableManager.listTableViewFilteredRows(dtable, table, view, filters, filterConjunction)});
        return;
      });
    });
    ...
    app.get('/api/v1/dtables/:dtable_uuid/operations', (req, res) => {
      ...
      DtableUtils.queryDtableOperations(dtable_uuid, (page-1)*count, count, (err, results) => {
        if (err) {
          logger.error(err);
          res.sendStatus(500);
          return;
    ...
    app.post('/api/v1/dtables/:dtable_uuid/operations', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload || (payload.permission !== 'rw')) {
        res.status(403).send({"error_msg": 'You don\'t have permission to update the current table.'});
        return;
      }
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        try {
          let { isValid, error_message } = dtableManager.execHttpOperation(username, app_name, dtable_uuid, dtable, operation);
          if (!isValid) {
            res.status(400).send(error_message);
            return;
          }
    ...
    app.delete('/api/v1/dtables/:dtable_uuid/comments/:comment_id/', (req, res) => {
      // params check
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({'error_msg': 'You don\'t have permission to delete comment'});
        return;
      }
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        commentManager.getRowComment(comment_id, (err, comment) => {
          if (err) {
            logger.error('user: ', payload.username, ' query such comment: ', comment_id, ' error: ', err);
            res.sendStatus(500);
            return;
          }
          if (!comment) {
            res.status(404).send({'error_msg': `comment ${comment_id} not found`});
            return;
          }
          ...
          commentManager.deleteRowComment(comment_id, (err) => {
            if (err) {
              logger.error('user: ', payload.username, ' delete comment: ', comment_id, ' error: ', err);
              res.sendStatus(500);
              return;
            }
            res.send({'success': true});
    ...
    app.put('/api/v1/dtables/:dtable_uuid/comments/:comment_id/', (req, res) => {
      // params check
      let { dtable_uuid, comment_id } = req.params;
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({'error_msg': 'You don\'t have permission to update the comment'});
        return;
      }
      // resource check and exec sql
      let dtableManager = appContext.getDTableManager();
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        commentManager.getRowComment(comment_id, (err, comment) => {
          if (err) {
            logger.error('user: ', payload.username, ' query comment: ', comment_id, ' when PUT comment error: ', err);
            res.sendStatus(500);
            return;
          }
          if (!comment) {
          ...
          if (comment.author !== payload.username && options.comment) {
            res.status(403).send({'error_msg': 'You don\'t have permission to edit comment'});
            return;
          }
          commentManager.updateRowComment(payload.username, comment_id, options, (err) => {
            if (err) {
              logger.error('user: ', payload.username, ' update comment: ', comment_id, ' with options: ', options, ' error: ', err);
              res.sendStatus(500);
              return;
            }
            res.send({'success': true});
    ...
    app.get('/api/v1/dtables/:dtable_uuid/comments/:comment_id/', (req, res) => {
      // params check
      let { dtable_uuid, comment_id } = req.params;
      if (isNaN(comment_id)) {
        res.status(400).send({'error_msg': 'comment_id invalid'});
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({'error_msg': 'You don\'t have permission to read comment'});
        return;
      }
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        commentManager.getRowComment(comment_id, (err, comment) => {
          if (err) {
            logger.error('user: ', payload.username, ' query single comment: ', comment_id, ' error: ', err);
            res.sendStatus(500);
            return;
          }
          if (!comment) {
          ...
          if(comment.dtable_uuid !== dtable_uuid) {
            res.status(403).send({'error_msg': 'You don\'t have permission to get the comment'});
            return;
          }
          res.send(comment);
        });
    ...
    app.post('/api/v1/dtables/:dtable_uuid/comments/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({'error_msg': 'You don\'t have permission to comment the row'});
        return;
      }
      // resource check and exec sql
      let dtableManager = appContext.getDTableManager();
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let commentManager = appContext.getCommentManager();
        commentManager.addRowComment(payload.username, dtable_uuid, row_id, comment, (err) => {
          if (err) {
            logger.error('user: ', payload.username, ' in dtable: ', dtable_uuid, ' row: ', row_id, ' create comment: ', comment, ' error: ', err);
            res.sendStatus(500);
            return;
          }
          ...
          for (let to_user of to_users) {
            ...
            notificationManager.addNotification(to_user.email, dtable_uuid, MSG_TYPE_ROW_COMMENT, JSON.stringify(detail), (err, results) => {
              if (err) {
                logger.error('add row_comment notification failed:', err);
              }
              let userManager = appContext.getUserManager();
              let socketIdList = userManager.getSocketIdList(dtable_uuid, to_user.email);
    ...
    app.get('/api/v1/dtables/:dtable_uuid/comments/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to read comments.'});
        return;
      }
      // exec sql and resp
      let dtableManager = appContext.getDTableManager();
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let commentManager = appContext.getCommentManager();
        commentManager.listRowComments(dtable_uuid, row_id, per_page, offset, (err, results) => {
          if (err) {
            logger.error('user: ', payload.username, ' query comments in dtable: ', dtable_uuid, ' row: ', row_id, ' error: ', err);
            res.sendStatus(500);
            return;
          }
          res.send(results);
    ...
    app.get('/api/v1/dtables/:dtable_uuid/comments-count/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to access the table\'s comments.'});
        return;
      }
      let dtableManager = appContext.getDTableManager();
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let count = 0;
        let commentManager = appContext.getCommentManager();
        commentManager.getRowCommentCount(dtable_uuid, row_id, (err, results) => {
          if (err) {
            logger.error('user: ', payload.username, ' query comments-count in dtable: ', dtable_uuid, ' row: ', row_id, ' error: ', err);
            res.sendStatus(500);
            return;
          }
    ...
    app.get('/api/v1/dtables/:dtable_uuid/comments-within-days/', (req, res) => {
      let { dtable_uuid } = req.params;
      let { days } = req.query;
      if (days && isNaN(days)) {
        res.status(400).send({'error_msg': 'days is invalid.'})
        return;
      }
      days = days && parseInt(days) > 0 ? parseInt(days) : 3;
      // permission check
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to read comments.'});
      ...
      dtableManager.getDtable(dtable_uuid, (err) => {
        ...
        let commentManager = appContext.getCommentManager();
        commentManager.listRowCommentsWithinDays(dtable_uuid, days, (err, comments) => {
          if (err) {
            logger.error(err);
            res.sendStatus(500);
            return;
    ...
    app.get('/api/v1/dtables/:dtable_uuid/related-users/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to get users.'});
        return;
      }
      let dtableManager = appContext.getDTableManager();
      dtableManager.getDtable(dtable_uuid, (err) => {
        ...
        dtableManager.getRelatedUsers(dtable_uuid, (err, users) => {
          if (err) {
            logger.error('get dtable related-users: ', dtable_uuid, ' error: ', err);
            res.sendStatus(500);
            return;
          }
    ...
    app.get('/api/v1/dtables/:dtable_uuid/activities/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to get activities.'});
        return;
      }
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        dtableManager.getRowActivities(dtable_uuid, row_id, per_page, offset, (err, activities, total_count) => {
          if (err) {
    ...
    app.get('/api/v1/dtables/:dtable_uuid/deleted-rows/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to get deleted rows.'});
        return;
      }
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        DtableUtils.getDtableDeletedRows(dtable_uuid, (page-1)*per_page, per_page, (err, results) => {
          if (err) {
    ...
    app.get('/api/v1/dtables/:dtable_uuid/notifications/', (req, res) => {
      ...
      let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
      if (!payload) {
        res.status(403).send({"error_msg": 'You don\'t have permission to access the table\'s notifications.'});
        return;
      }
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let notificationManager = appContext.getNotificationManager();
        notificationManager.listNotifications(payload.username, dtable_uuid, (page-1)*per_page, per_page, (err, results) => {
          if (err) {
            logger.error(err);
            res.sendStatus(500);
            return;
    ...
    app.post('/api/v1/dtables/:dtable_uuid/notifications/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let notificationManager = appContext.getNotificationManager();
        notificationManager.addNotification(to_user, dtable_uuid, msg_type, JSON.stringify(detail), (err, results) => {
          if (err) {
            logger.error(err);
            res.sendStatus(500);
            return;
    ...
    app.put('/api/v1/dtables/:dtable_uuid/notifications/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let notificationManager = appContext.getNotificationManager();
        notificationManager.updateNotifications(payload.username, dtable_uuid, seen, (err, results) => {
          if (err) {
            logger.error(err);
            res.sendStatus(500);
            return;
    ...
    app.put('/api/v1/dtables/:dtable_uuid/notifications/:notification_id/', (req, res) => {
      // parameters check
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let notificationManager = appContext.getNotificationManager();
        notificationManager.getNotification(payload.username, notification_id, (err, notification) => {
          if (err) {
            logger.error(err);
            res.sendStatus(500);
            return;
          ...
          if (!notification) {
            res.status(404).send({'error_msg': `notification ${notification_id} not found.`});
            return;
          }
          notificationManager.updateNotification(payload.username, notification_id, seen, (err, results) => {
            if (err) {
              logger.error(err);
              res.sendStatus(500);
              return;
    ...
    app.delete('/api/v1/dtables/:dtable_uuid/notifications/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (err, dtable) => {
        ...
        let notificationManager = appContext.getNotificationManager();
        notificationManager.deleteNotifications(payload.username, dtable_uuid, (err, results) => {
          if (err) {
            logger.error(err);
            res.sendStatus(500);
            return;
    ...
    app.post('/api/v1/:dtable_uuid/import-csv/', multiMiddleware, (req, res) => {
      try {
        let dtable_uuid = req.params.dtable_uuid;
        let payload = decodeAuthorization(req.headers.authorization, dtable_uuid);
        if (!payload || payload.permission !== 'rw') {
          res.status(403).send({'error_msg': 'You don\'t have permission to import table'});
          return;
        }
        let { username } = payload;

        // if is_create_base is true, init Table1 should be deleted after import
        let { table_name, lang, is_create_base } = req.body;
        if (!table_name) {
        ...
        dtableManager.getDtable(dtable_uuid, (err, dtable) => {
          ...
          let originalEncoding = detectCharacterEncoding(buffer);
          let isOriginEncodingUTF8 = true;
          if (originalEncoding.encoding !== 'UTF-8' && originalEncoding.encoding !== 'utf-8') {
            logger.info('originalEncoding: %o', originalEncoding);
          ...
          if (!isOriginEncodingUTF8) {
            stream = stream.pipe(iconv.decodeStream(originalEncoding.encoding)).pipe(iconv.encodeStream('utf-8'));
          }
          stream.pipe(csv.parse({
            ...
            try {
              this.dtableServer.dtableManager.insertTableWithRawColumnsAndRows(dtable, dtable_uuid, table_name, raw_cols, raw_rows, lang, username);
            } catch(err) {
            ...
            for (let file_field in req.files) {
              if (req.files[file_field] instanceof Array) {
                ...
                if (fs.existsSync(req.files[file_field].path)) fs.unlinkSync(req.files[file_field].path);
              }
            }

            if (is_create_base) {
              this.dtableServer.dtableManager.deleteTableById(dtable, "0000");
            }
    ...
    app.get('/api/v1/internal/:dtable_uuid/connected-apps/', (req, res) => {
      ...
      dtableManager.getDtable(dtable_uuid, (error, dtable) => {
        ...
        let webSocketManager = appContext.getWebSocketManager();
        let connectedApps = webSocketManager.getDTableConnectedApps(dtable_uuid);

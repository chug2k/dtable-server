...
class EventManager {
      ...
      for (row_id of operation.row_ids) {
        ...
        for (let column of table.columns) {
          if (operation.updated[row_id][column.key] !== undefined) {
            ...
            if (column.type === 'link') {
              cell_data = this.getFormattedLinkCellData(dtable, column, cell_data, value, old_value);
            }
  ...
  getFormattedLinkCellData(dtable, column, cell_data, value, old_value=null) {
    let link_table = TableUtils.getTableById(dtable.value.tables, column.data.other_table_id);
    let new_cell_data = cell_data;
    let link_value = [];
    for (let link_row_id of value) {
      let link_row = TableUtils.getRowById(link_table, link_row_id);
      let link_row_name = link_row["0000"] ? link_row["0000"] : '';
      link_value.push({[link_row_id]: link_row_name})
    }
    new_cell_data.value = link_value;
    if (old_value !== null) {
      let link_old_value = [];
      for (let link_row_id of old_value) {
        let link_row = TableUtils.getRowById(link_table, link_row_id);
        let link_row_name = link_row["0000"] ? link_row["0000"] : '';
        link_old_value.push({[link_row_id]: link_row_name})
      }

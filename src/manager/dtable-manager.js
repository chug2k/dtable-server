...
import { v4 } from 'uuid';
import { OPERATION_TYPE, Operation, Views, TableUtils, RowUtils, LinksUtils, GridTable, GridRow, GridColumn, CellType, generatorTableId, generatorColumnKey } from "dtable-store";
...
class DTableManager {
      ...
      DtableUtils.loadDtableData(uuid, (err, results) => {
        if (err) {
          ...
          let error = {
            ...
            'error_message': `dtable_uuid: ${uuid} - ` + ERROR_MESSAGE[errorType]
          };
  ...
  execHttpOperation(username, appName, dtable_uuid, dtable, operation) {
    ...
    // record
    let version = dtable.value.version;
    DtableUtils.recordOperation(dtable_uuid, version, newOperation, username, appName);
  ...
  execSocketOperation(username, appName, dtable_uuid, dtable, operation) {

    // valid
    let isValid = this.canOpApply(dtable_uuid, operation);
    ...
    // execute
    this.updateDTable(dtable, newOperation);
  ...
  addLinkToTableRow(username, appName, dtableUuid, dtable, linkParams, callback) {
    let { table_name, other_table_name, table_row_id, other_table_row_id } = linkParams;

    // resource check
    let table = TableUtils.getTableByName(dtable.value.tables, table_name);
    let otherTable = TableUtils.getTableByName(dtable.value.tables, other_table_name);
    if (!table || !otherTable) {
      let error_message = {
        error_type: 'table_not_exist',
        error_message: `table ${table_name} or ${other_table_name} not found`
      }
    ...
    let operation = LinksUtils.transLinkOptions(table._id, otherTable._id, table_row_id, other_table_row_id);
    operation = Object.assign({}, operation, {op_type: OPERATION_TYPE.ADD_LINK});
  ...
  deleteTableById(dtable, tableId) {
    TableUtils.deleteTable(dtable.value.tables, tableId);
  }

  deleteLinkFromTableRow(username, appName, dtableUuid, dtable, linkParams, callback) {
    let { table_name, other_table_name, table_row_id, other_table_row_id } = linkParams;
    ...
    let operation = LinksUtils.transLinkOptions(table._id, otherTable._id, table_row_id, other_table_row_id);
    operation = Object.assign({}, operation, {op_type: OPERATION_TYPE.REMOVE_LINK});

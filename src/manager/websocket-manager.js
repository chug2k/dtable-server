...
class WebSocketManager {
  ...
  onConnected(socket) {
    ...
    socket.on('update-dtable', (dtable_uuid, operation, callback) => {
      ...
      let dtable = this.dtableManager.dtables.get(dtable_uuid);

      if (dtable) {
        let { username, appName } = user;
        let { isValid } = this.dtableManager.execSocketOperation(username, appName, dtable_uuid, dtable, JSON.parse(operation));
        if (!isValid) {
        ...
        let nextVersion = dtable.value.version;
        ...
        logger.error(`DTable ${dtable_uuid} can't be loaded.`);

...
class OperationUtils {
  ...
  static checkOperation(dtable, operation) {
    ...
    switch (op_type) {
      ...
      case OPERATION_TYPE.ADD_LINK: 
      case OPERATION_TYPE.REMOVE_LINK: {
        let needParams = ['table1_id', 'table2_id', 'table1_row_id', 'table2_row_id'];
        let params = Object.keys(operation);
        ({ isValid, error_message } = this._checkOperationParams(needParams, params));
        ...
        let { table1_id, table2_id } = operation;

        // check table1
        ({ isValid, error_message } = this._checkTable(dtable, table1_id));
        if (!isValid) {
          return { isValid, error_message };
        }
        
        // check table2
        ({ isValid, error_message } = this._checkTable(dtable, table2_id));
        if (!isValid) {
          return { isValid, error_message };
        }
        
        let { table1_row_id, table2_row_id } = operation;

        // check row1
        ({ isValid, error_message } = this._checkRow(dtable, table1_id, table1_row_id));
        if (!isValid) {
          return { isValid, error_message };
        }
        
        // check row2
        ({ isValid, error_message } = this._checkRow(dtable, table2_id, table2_row_id));
        return { isValid, error_message };
      }
